...
def create_mockup_data():
    ...
    team1_members = [
        ('ic101', 'password1', 'สมชาย ใจดี', 'Team1'),
        ('ic102', 'password2', 'สมหญิง รักงาน', 'Team1'),
        ('ic103', 'password3', 'สมศักดิ์ ขยัน', 'Team1'),
    ]
    team2_members = [
        ('ic201', 'password4', 'สุดา สมใจ', 'Team2'),
        ('ic202', 'password5', 'วิไล เก่งมาก', 'Team2'),
        ('ic203', 'password6', 'ประยุทธ์ ดีเด่น', 'Team2'),
        ('ic204', 'password7', 'ยิ่งลักษณ์ สวยงาม', 'Team2'),
        ('ic205', 'password8', 'ทักษิณ เก่งกาจ', 'Team2'),
    ]

    for username, password, full_name, hub in team1_members + team2_members:
        users_df = pd.concat([
            users_df,
            pd.DataFrame([{ 
                'user_id': str(uuid.uuid4()),
    ...
    campaigns_data = [
        {
            ...
            'campaign_name': 'IPO กอง B-CNNEXT',
            'campaign_type': 'IPO',
            'description': 'กองทุนรวมหุ้นระยะยาวที่ลงทุนในหุ้นไทยที่มีศักยภาพการเติบโตสูง',
        ...
        },
        {
            ...
            'campaign_name': 'กรมธรรม์ครบกำหนด Sep2025',
            'campaign_type': 'Insurance',
            'description': 'แคมเปญติดต่อลูกค้ากรมธรรม์ที่ครบกำหนดในเดือนกันยายน 2025',
        ...
        },
        {
            ...
            'campaign_name': 'หุ้นกู้ครบกำหนด June2025',
            'campaign_type': 'Bond',
            'description': 'แคมเปญติดต่อลูกค้าหุ้นกู้ที่ครบกำหนดในเดือนมิถุนายน 2025',
    ...
    for c in campaigns_data:
        campaigns_df = pd.concat([campaigns_df, pd.DataFrame([c])], ignore_index=True)

    # Leads mock (10 per IC)
    all_members = team1_members + team2_members
    statuses = ['ยังไม่ติดต่อ', 'ติดต่อแล้ว', 'ปิดการขายสำเร็จ', 'รอตัดสินใจ', 'ไม่สนใจ', 'ติดต่อไม่ได้']
    priorities = ['High', 'Medium', 'Low']

    leads_rows: list[dict] = []
    for idx, (username, _, full_name, hub) in enumerate(all_members):
        hub_name = f"Team{1 if idx < 3 else 2}"
        for n in range(10):
            camp = campaigns_data[n % 3]
            policy_name = None
            maturity_date = None
            maturity_amount = None
            if camp['campaign_id'] == 'CAMP-002':  # Insurance
                policy_name = f"กรมธรรม์ประกันชีวิต {random.randint(1, 5)} ปี"
                maturity_date = f"2025-09-{random.randint(1, 30):02d}"
                maturity_amount = random.randint(100_000, 1_000_000)
            elif camp['campaign_id'] == 'CAMP-003':  # Bond
                policy_name = f"หุ้นกู้ {random.randint(3, 10)} ปี"
                maturity_date = "2025-06-15"
                maturity_amount = random.randint(50_000, 500_000)

            leads_rows.append({
                'lead_id': str(uuid.uuid4()),
                'campaign_id': camp['campaign_id'],
                'customer_name': f"ลูกค้า{idx*10+n+1}",
                'phone': f"08{random.randint(10000000, 99999999)}",
                'email': f"customer{idx*10+n+1}@email.com",
                'birth_date': None,
                'investment_level': random.choice(['Beginner', 'Intermediate', 'Expert']),
                'previous_product': None,
                'investment_budget': random.randint(100_000, 5_000_000),
                'preferred_contact': random.choice(['Phone', 'Line', 'Email']),
                'policy_name': policy_name,
                'maturity_date': maturity_date,
                'maturity_amount': maturity_amount,
                'assigned_hub': hub_name,
                'assigned_ic': username,
    ...
    leads_df = pd.DataFrame(leads_rows, columns=LEAD_SCHEMA_COLS)
